...
export default function Rentals() {
    ...
    return (
      <div className="min-h-screen">
        ...
        <main className="container mx-auto px-6 py-12">
          <h1 className="text-4xl font-bold mb-8 text-junot-text-dark">{t('rentals.title')}</h1>
          <div className="modern-card p-12 text-center">
            <div className="text-6xl mb-4">🔒</div>
            <h3 className="text-2xl font-bold mb-4 text-junot-text-dark">{t('common.connectWallet')}</h3>
            <p className="text-junot-text-muted">{t('rentals.connectDesc') || 'Connect your wallet to manage rental payments'}</p>
          </div>
        </main>
      </div>
    );
  ...
  return (
    <div className="min-h-screen">
      ...
      <main className="container mx-auto px-6 py-12">
        <div className="mb-8">
          <h1 className="text-4xl font-bold mb-2 text-junot-text-dark">{t('rentals.title')}</h1>
          <p className="text-junot-text-muted">{t('rentals.subtitle')}</p>
        </div>

        {/* Stats */}
        <div className="grid md:grid-cols-3 gap-6 mb-8">
          <div className="modern-card p-6">
            <div className="text-junot-text-muted text-sm mb-2 uppercase tracking-wider">{t('rentals.stats.asTenant')}</div>
            <div className="text-3xl font-bold text-junot-text-dark">{mockTenantAgreements.length}</div>
            <div className="text-sm text-junot-text-muted mt-1">{t('rentals.stats.rentalAgreements')}</div>
          </div>
          <div className="modern-card p-6">
            <div className="text-junot-text-muted text-sm mb-2 uppercase tracking-wider">{t('rentals.stats.asLandlord')}</div>
            <div className="text-3xl font-bold text-junot-text-dark">{mockLandlordAgreements.length}</div>
            <div className="text-sm text-junot-text-muted mt-1">{t('rentals.stats.rentalAgreements')}</div>
          </div>
          <div className="modern-card p-6">
            <div className="text-junot-text-muted text-sm mb-2 uppercase tracking-wider">{t('rentals.stats.totalCollected')}</div>
            <div className="text-3xl font-bold text-junot-gold">{formatPrice(mockIncomeHistory.reduce((sum, i) => sum + i.amount, 0))}</div>
            <div className="text-sm text-junot-text-muted mt-1">{t('rentals.stats.totalRevenue')}</div>
          </div>
        </div>
        ...
        <div className="flex gap-4 mb-6">
          ...
          >
            {t('rentals.tabs.tenant')} ({mockTenantAgreements.length})
          </button>
          <button
          ...
          >
            {t('rentals.tabs.landlord')} ({mockLandlordAgreements.length})
          </button>
        </div>
        ...
        {view === 'tenant' && (
          <div className="space-y-6">
              ...
              return (
                <div key={agreement.id} className="modern-card p-6">
                  <div className="flex items-center justify-between mb-6">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        ...
                        <p className="text-junot-text-muted text-sm mt-1">{t('rentals.agreement.landlord')}: {agreement.landlord}</p>
                      </div>
                    </div>
                    ...
                    <div className={`px-4 py-2 rounded-lg ${
                      ...
                      {t('rentals.agreement.active')}
                    </div>
                  </div>

                  <div className="grid md:grid-cols-4 gap-6 mb-6">
                    <div>
                      <div className="text-junot-text-muted text-sm mb-1">{t('rentals.agreement.monthlyRent')}</div>
                      <div className="text-2xl font-bold text-junot-text-dark">{formatPrice(agreement.rentAmount)}</div>
                    </div>
                    <div>
                      <div className="text-junot-text-muted text-sm mb-1">{t('rentals.agreement.nextPayment')}</div>
                      <div className={`text-2xl font-bold ${isOverdue ? 'text-red-600' : 'text-junot-text-dark'}`}>
                        {isOverdue ? t('rentals.agreement.overdue') : `${daysUntilDue} ${t('rentals.agreement.days')}`}
                      </div>
                    </div>
                    <div>
                      <div className="text-junot-text-muted text-sm mb-1">{t('rentals.agreement.lastPayment')}</div>
                      <div className="text-lg font-semibold text-junot-text-dark">
                        {new Date(agreement.lastPayment).toLocaleDateString()}
                      </div>
                    </div>
                    <div>
                      <div className="text-junot-text-muted text-sm mb-1">{t('rentals.agreement.totalPayments')}</div>
                  ...
                  <div className="flex gap-4">
                    ...
                    >
                      {isOverdue ? `⚠️ ${t('rentals.agreement.payOverdue')}` : daysUntilDue <= 5 ? t('rentals.agreement.payRent') : `${t('rentals.agreement.dueIn')} ${daysUntilDue} ${t('rentals.agreement.days')}`}
                    </button>
                    <button
                    ...
                    >
                      {t('rentals.agreement.viewDetails')}
                    </button>
                  </div>
                </div>
              );
            ...
            {mockTenantAgreements.length === 0 && (
              <div className="modern-card p-12 text-center">
                <div className="text-6xl mb-4">🏠</div>
                <h3 className="text-xl font-bold mb-2">{t('rentals.noRentals')}</h3>
                <p className="text-neutral-400">{t('rentals.noRentalsDesc')}</p>
              </div>
            )}
          </div>
        )}
        ...
        {view === 'landlord' && (
          <div className="space-y-6">
            {/* Active Agreements */}
            <div>
              <h2 className="text-2xl font-bold mb-4">{t('rentals.landlordView.activeAgreements')}</h2>
              {mockLandlordAgreements.map((agreement) => {
                ...
                return (
                  <div key={agreement.id} className="modern-card p-6 mb-4">
                    <div className="flex items-center justify-between mb-6">
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          ...
                          <p className="text-neutral-400 text-sm mt-1">{t('rentals.agreement.tenant')}: {agreement.tenant}</p>
                        </div>
                      </div>
                    ...
                    <div className="grid md:grid-cols-4 gap-6 mb-6">
                      <div>
                        <div className="text-neutral-400 text-sm mb-1">{t('rentals.agreement.monthlyRent')}</div>
                        <div className="text-2xl font-bold text-green-400">{formatPrice(agreement.rentAmount)}</div>
                      </div>
                      <div>
                        <div className="text-neutral-400 text-sm mb-1">{t('rentals.landlordView.nextExpected')}</div>
                        <div className="text-lg font-semibold">{daysUntilDue} {t('rentals.agreement.days')}</div>
                      </div>
                      <div>
                        <div className="text-neutral-400 text-sm mb-1">{t('rentals.landlordView.lastReceived')}</div>
                        <div className="text-lg font-semibold">
                          {new Date(agreement.lastPayment).toLocaleDateString()}
                        </div>
                      </div>
                      <div>
                        <div className="text-neutral-400 text-sm mb-1">{t('rentals.landlordView.totalCollected')}</div>
            ...
            {/* Income History */}
            <div>
              <h2 className="text-2xl font-bold mb-4">{t('rentals.landlordView.incomeHistory')}</h2>
              <div className="modern-card overflow-hidden">
                <table className="w-full">
                  <thead className="bg-junot-gold/10">
                    <tr>
                      <th className="text-left p-4 font-semibold text-junot-text-dark">{t('rentals.landlordView.property')}</th>
                      <th className="text-left p-4 font-semibold text-junot-text-dark">{t('rentals.landlordView.tenant')}</th>
                      <th className="text-left p-4 font-semibold text-junot-text-dark">{t('rentals.landlordView.amount')}</th>
                      <th className="text-left p-4 font-semibold text-junot-text-dark">{t('rentals.landlordView.date')}</th>
                      <th className="text-left p-4 font-semibold text-junot-text-dark">{t('rentals.landlordView.status')}</th>
                    </tr>
                  </thead>
                  <tbody>
          ...
          <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-6" onClick={() => setSelectedAgreement(null)}>
            <div className="bg-junot-cream-light/95 backdrop-blur-xl rounded-2xl border border-junot-border shadow-2xl max-w-3xl w-full max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
              ...
              <div className="sticky top-0 bg-junot-cream/90 backdrop-blur-xl border-b border-junot-border p-6 flex items-center justify-between">
                <h2 className="text-2xl font-bold text-junot-text-dark">{t('rentals.modal.title')}</h2>
                <button
              ...
              <div className="p-6 space-y-6">
                ...
                <div className="flex items-center gap-4 p-4 bg-white/60 backdrop-blur-sm rounded-xl">
                  ...
                  <div>
                    ...
                    <p className="text-neutral-400 text-sm mt-1">
                      {selectedAgreement.viewType === 'tenant' ? `${t('rentals.agreement.landlord')}: ${selectedAgreement.landlord}` : `${t('rentals.agreement.tenant')}: ${selectedAgreement.tenant}`}
                    </p>
                  </div>
                </div>
                ...
                {/* Agreement Terms */}
                <div>
                  <h3 className="text-lg font-bold mb-3">{t('rentals.modal.terms')}</h3>
                  <div className="grid grid-cols-2 gap-4">
                    <div className="bg-white/60 backdrop-blur-sm p-4 rounded-xl">
                      <div className="text-neutral-400 text-sm mb-1">{t('rentals.modal.monthlyRent')}</div>
                      <div className="text-2xl font-bold text-junot-text-dark">{formatPrice(selectedAgreement.rentAmount)}</div>
                    </div>
                    <div className="bg-white/60 backdrop-blur-sm p-4 rounded-xl">
                      <div className="text-neutral-400 text-sm mb-1">{t('rentals.modal.paymentFrequency')}</div>
                      <div className="text-2xl font-bold">{selectedAgreement.paymentFrequency} {t('rentals.agreement.days')}</div>
                    </div>
                    <div className="bg-white/60 backdrop-blur-sm p-4 rounded-xl">
                      <div className="text-neutral-400 text-sm mb-1">{t('rentals.modal.agreementStatus')}</div>
                      <div className="text-lg font-bold text-green-400">{t('rentals.agreement.active')}</div>
                    </div>
                    <div className="bg-white/60 backdrop-blur-sm p-4 rounded-xl">
                      <div className="text-neutral-400 text-sm mb-1">{t('rentals.modal.totalPayments')}</div>
                ...
                {/* Payment Schedule */}
                <div>
                  <h3 className="text-lg font-bold mb-3">{t('rentals.modal.paymentSchedule')}</h3>
                  <div className="bg-white/60 backdrop-blur-sm p-4 rounded-xl space-y-3">
                    <div className="flex justify-between items-center">
                      <span className="text-neutral-400">{t('rentals.modal.lastPayment')}:</span>
                      <span className="font-semibold">{new Date(selectedAgreement.lastPayment).toLocaleDateString()}</span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span className="text-neutral-400">{t('rentals.modal.nextDue')}:</span>
                      <span className="font-semibold text-junot-text-dark">
                        {new Date(selectedAgreement.nextDueDate).toLocaleDateString()}
                      </span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span className="text-neutral-400">{t('rentals.modal.daysUntil')}:</span>
                      <span className={`font-semibold ${getDaysUntilDue(selectedAgreement.nextDueDate) < 0 ? 'text-red-400' : 'text-green-400'}`}>
                        {getDaysUntilDue(selectedAgreement.nextDueDate) < 0 ? t('rentals.agreement.overdue') : `${getDaysUntilDue(selectedAgreement.nextDueDate)} ${t('rentals.agreement.days')}`}
                      </span>
                    </div>
                  </div>
                </div>
                ...
                {/* Payment History */}
                <div>
                  <h3 className="text-lg font-bold mb-3">{t('rentals.modal.paymentHistory')}</h3>
                  <div className="bg-white/60 backdrop-blur-sm rounded-xl overflow-hidden">
                    <table className="w-full">
                      <thead className="bg-junot-gold/10">
                        <tr>
                          <th className="text-left p-3 text-sm font-semibold text-junot-text-dark">{t('rentals.modal.paymentNum')}</th>
                          <th className="text-left p-3 text-sm font-semibold text-junot-text-dark">{t('rentals.modal.amount')}</th>
                          <th className="text-left p-3 text-sm font-semibold text-junot-text-dark">{t('rentals.modal.date')}</th>
                          <th className="text-left p-3 text-sm font-semibold text-junot-text-dark">{t('rentals.modal.status')}</th>
                        </tr>
                      </thead>
                      <tbody>
                ...
                {/* Financial Summary */}
                <div>
                  <h3 className="text-lg font-bold mb-3">{t('rentals.modal.financialSummary')}</h3>
                  <div className="grid grid-cols-2 gap-4">
                    <div className="bg-white/60 backdrop-blur-sm p-4 rounded-xl">
                      <div className="text-neutral-400 text-sm mb-1">{t('rentals.modal.totalPaid')}</div>
                      <div className="text-2xl font-bold text-green-400">
                        {formatPrice(selectedAgreement.rentAmount * selectedAgreement.totalPayments)}
                      </div>
                    </div>
                    <div className="bg-white/60 backdrop-blur-sm p-4 rounded-xl">
                      <div className="text-neutral-400 text-sm mb-1">{t('rentals.modal.avgMonthly')}</div>
                ...
                <div className="flex gap-4 pt-4 border-t border-neutral-800">
                  ...
                  <button className="flex-1 py-3 bg-red-600/20 hover:bg-red-600/30 text-red-400 rounded-lg font-semibold transition-colors border border-red-500/50">
                    {t('rentals.modal.terminate')}
                  </button>
                  <button
                    onClick={() => setSelectedAgreement(null)}
                    className="px-6 py-3 bg-white/60 border border-junot-border hover:bg-white/80 rounded-lg font-semibold transition-colors"
                  >
                    {t('rentals.modal.close')}
                  </button>
                </div>
              </div>
            </div>
